 ...
 * the file for the port and the keepalived triggers, so a container can move a port or
 * switch a trigger on without a mounted config file. And the two tolerated settings --
 * `logLevel` and `allowSelfSignedCertificates` -- lose a wrong value with a warning here,
 * so the schema behind this can stay strict about everything it does see.
 */
function prepare(raw: unknown): Record<string, unknown> {
    ...
    const port = process.env.KASM_CLIENT_PORT?.trim();
    ...
    const keepalived: Record<string, unknown> =
        out.keepalived && typeof out.keepalived === "object"
            ? { ...(out.keepalived as Record<string, unknown>) }
            : {};
    const fifo = process.env.KASM_NOTIFY_FIFO?.trim();
    if (fifo) keepalived.notifyFifo = fifo;
    const notifyToken = process.env.KASM_NOTIFY_TOKEN?.trim();
    if (notifyToken) keepalived.notifyToken = notifyToken;
    out.keepalived = keepalived;
    ...
    if (out.registrationSecret !== undefined && out.registrationSecret !== null) {
        logger.warn(
            "Ignoring registrationSecret in config.yaml: it is no longer read. Register with the " +
                "setup PIN from this log, or set KASM_REGISTRATION_SECRET instead.",
        );
    }
 ...
 * where nobody reads the agent's log. Environment only -- `KASM_REGISTRATION_SECRET`, or
 * `KASM_REGISTRATION_SECRET_FILE` naming a file that holds it (a Docker or Podman secret).
 *
...
function readRegistrationSecret(): string | null {
    const value = process.env.KASM_REGISTRATION_SECRET?.trim() || null;
    const file = process.env.KASM_REGISTRATION_SECRET_FILE?.trim() || null;
    if (value && file) {
        logger.fatal(
            "Set KASM_REGISTRATION_SECRET or KASM_REGISTRATION_SECRET_FILE, not both",
        );
        process.exit(1);
    }
    ...
    try {
        ...
        const code = (err as NodeJS.ErrnoException).code;
        logger.fatal(`KASM_REGISTRATION_SECRET_FILE cannot be read at ${file}${code ? ` (${code})` : ""}`);
        process.exit(1);
    }
    if (!content) {
        logger.fatal(`KASM_REGISTRATION_SECRET_FILE is empty: ${file}`);
        process.exit(1);
    }
    return content;
}
